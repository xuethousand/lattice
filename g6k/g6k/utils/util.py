...
def load_svpchallenge_and_randomize(n, s=None, seed=None, verbose=True, float_type="double"):
    ...
    :param seed: seed for rerandomization 不用这个随机化参数了
    :returns: lattice basis and BKZ object
    ...
    if not os.path.isfile(filename):
        logging.info("Did not find '{filename}', generating ...".format(filename=filename))
        command = "/workspaces/lattice/generator/generate_random --dim {} --seed {}".format(n, s)
        try:
            result = subprocess.run(command, check=True, shell=True, stdout=subprocess.PIPE)
            with open(filename, 'w') as f:
                f.write(result.stdout.decode())
        except subprocess.CalledProcessError as e:
            logging.error("Failed to generate SVP challenge: {}".format(e))
            raise

  #  return load_matrix_file(filename, randomize=True, seed=seed, float_type=float_type)
    return load_matrix_file(filename, randomize=False, float_type=float_type)
