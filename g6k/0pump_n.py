...
logging.basicConfig(filename=f'pump_n/logging_pump_n.txt', filemode = 'a', level=logging.INFO)



def usvp():
    """
    确认pump_n与TT、approx factor的关系
    """
    goal_appro_factor = 0   #避免goal_appro_factor达到goal、norm未达到，就在workout阶段终止的情况，所以这里设置很小的值
    goal_norm = 1**2  #目标长度控制终止程序
    ...
    for dim in range(40,65,5):
        ...
        while(trial < trial_n):
            ...
            flast = workout_modified(g6k, tracer, kappa=0, blocksize=dim, goal_r0=goal_r0,dim4free_min=0, pump_n=pump_n,  logfile = f'pump_n/logging_pump_n.txt',         # Main parameters
